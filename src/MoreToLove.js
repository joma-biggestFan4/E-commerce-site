            ...
            product.map(products => {
              return (
                <div className="topP" key={products.name}>
                  <div className="p-img">
                    <img src={products.cart_type} className="no-back" 
                    onClick={() => add_to_cart(products)} />
                  </div>
                  <div className="centerAlign">
                    <img className="not-proceed" src={products.img_src} alt="cart with orange background" />
                  </div>
                  <div className="padding">
                    <p className="no-padding">{products.name}</p>
                    <p className="no-padding p paddingB">$50.00 <span className="small">In stock</span></p>
                    {
                      products.isRated ?   <img src={fiveStar} className="five-star" /> : <p className="no-padding some-bisque">{products.discount}</p>
                    }
                  </div>
                </div>
              )
